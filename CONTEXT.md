...
**Session index**: The metadata verbs (`jb log`, `jb status`) and `jb ui` scan `.jb/sessions/*/metadata.json` — the files themselves are the index. `jb ui` serves it over HTTP as `GET /api/sessions`; there is no separate lister script.

**ID resolution**: Everywhere a session ID is accepted — `full uuid | unique 4+ hex prefix | @`. `@` = `$JB_SESSION`.

_Avoid_: Plugin, extension, module, TUI (jb has none of these). GUI is deliberately excepted in one place: `jb ui`, the read-only session forest viewer (see UI below).
...
- The **session tree** is an observability artifact — sessions linked by `parent`/`author` in metadata. Roots are sessions with no parent. Reconstructed from metadata by `jb log --graph` and `jb ui`. No runtime coordination.
...
The session forest viewer: a tiny read-only HTTP server on 127.0.0.1 (default port 8787, `--port 0` = random) serving the embedded `ui/` assets and two JSON endpoints over the session index:
- `GET /api/sessions` — every session: uuid, status, subject, author, parent, started_ms, ended_ms (only when finished), exit_code
- `GET /api/session/<uuid>` — the raw metadata.json of one session (exact uuid; unknown → 404)

The browser polls `/api/sessions` every 2s and renders the forest: solid edge = fork (parent link), dashed card = spawned (author link, no parent), roots standalone. Cards carry id/subject/status/age; clicking fetches the detail (turns, tokens, working_dir, lineage) into the bottom bar. **Read-only by design**: no writes, no provider calls, no config — the terminal is the controller (this is the deliberate GUI exception to the avoid list above).

**Source of truth**: `ui/` (index.html, style.css, app.js) is embedded into the binary at build time as `src/ui_assets.inc` (generated by `ui/gen-assets.sh`, same pattern as the pi viewer assets). `jb ui --dev` serves `ui/` from the repo root instead, for iteration. The server is single-threaded, per-connection close, 5s recv timeout — a local viewer, not a web server. Auto-opens the browser only when stdout is a TTY.
